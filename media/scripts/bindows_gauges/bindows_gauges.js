...
return r;};if(!Array.prototype.indexOf){Array.prototype.indexOf=function(obj,fromIndex){var l=this.length;if(fromIndex==null){fromIndex=0;}else if(fromIndex<0){fromIndex=Math.max(0,l+fromIndex);}
for(var i=fromIndex;i<l;i++){if(this[i]===obj)
return i;}
return-1;};}
...
{j--;delete sb[j];sb.length=j;continue;}
sb[j++]=segments[i];}
...
{if(this._disposed)return;BiObject.prototype.dispose.call(this);delete this._target;delete this._currentTarget;delete this._bubbles;delete this._propagationStopped;delete this._defaultPrevented;};_p.getDefaultPrevented=function()
...
var sb=[];for(var i in oNamespaces){sb.push("xmlns:",i,"=\"",oNamespaces[i],"\" ");}
...
window.attachEvent("onunload",this._onunload);else window.addEventListener("unload",this._onunload,false);};_p._findRootPath=function(){var els=document.getElementsByTagName("script");var l=els.length;var p,src;var re= /(^|\/)js\/application\.js$/;for(var i=0;i<l;i++){src=els[i].src;if(re.test(src)){p=RegExp.leftContext;if(p=="")
p="./";else if(p.charAt(p.length-1)!="/"){p+="/";}
...
else{if(this._shownDialogs.length==0){if(gp._parent==this){this.remove(gp);gp.setZIndex(0);gp.setTabIndex(-1);gp.getFocusRoot=function(){return this._parent;};gp.isFocusRoot=function(){return false;};var c=this.getActiveComponent();if(c&&c.getCanFocus()){c.setFocused(true);}}}}};_p.getGlassPaneVisible=function()
...
{return this._eventManager.setAllowBrowserContextMenu(b);};_p._create=function(oWindow){this._window=oWindow||window;this._document=this._window.document;if(BiBrowserCheck.ie&&!this._document.namespaces["v"]){this._document.namespaces.add("v","urn:schemas-microsoft-com:vml");}
var el=this._element=this._document.body;el._biComponent=this;this._window._biComponent=this;this._document.documentElement._biComponent=this;this._eventManager.attachToWindow(this._window);this._setHtmlProperties();this._setCssProperties();if(BiBrowserCheck.ie)
...
{case BiKeyboardEvent.ENTER:if(this._acceptButton&&this._acceptButton.getEnabled()&&!t.getAcceptsEnter()&&!e.getAltKey())
{this._acceptButton.dispatchEvent("action");if(this._acceptButton.getCommand())
{this._acceptButton.getCommand().execute()}}
break;case BiKeyboardEvent.ESC:if(this._cancelButton&&this._cancelButton.getEnabled()&&!t.getAcceptsEsc())
...
if(nameSb.length>0)
f.setName(nameSb.join(" "));return f;};(function()
...
var s=script.src;var base=s.substring(0,s.indexOf(packagedName));var str1="<script type=\"text/javascript\" src=\""+base;var str2=".js\"></script>";if(BiBrowserCheck.ie)document.write(str1+"gauge2.ie"+str2);if(BiBrowserCheck.moz)document.write(str1+"gauge2.moz"+str2);})();if(BiBrowserCheck.ie){var ss=document.createStyleSheet();ss.addRule("v\\:*","behavior:	url(#default#VML);");}
bindows={loadGaugeIntoDiv:function(sUri,divId){if(!application._window){application._adfPath=new BiUri(application._uri,"./");application._window=new BiApplicationWindow;application._window._onkeyevent=Function.EMPTY;application._window._create();}
var xmlHttp=new BiXmlHttp;xmlHttp.open('GET',sUri,false);xmlHttp.setRequestHeader("Accept-Encoding","gzip, deflate");xmlHttp.send(null);var doc=xmlHttp.responseXML;var s=xmlHttp.responseText;s=s.replace(/<\?xml[^\?]*\?>/,"");doc.loadXML(s);var gaugeClass=BiXmlResourceParser.getClassFromDocument(doc);var gauge=new gaugeClass();gauge.setVisible(false);var c=new BiInlineComponent(divId);c.add(gauge);application._window.add(c);BiTimer.callOnce(function(){gauge.setVisible(true);})
var g={__gauge:gauge}
var ids=gauge._xmlResourceParser._componentsById.getKeys();for(var i=0;i<ids.length;i++)
g[ids[i]]=gauge.getComponentById(ids[i]);return g;}}
